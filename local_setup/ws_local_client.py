...
def generate_full_dh_key(server_key):
    ...
    custom_print("[INFO] Shared key established")
    return shared_key
...
async def perform_task():
    ...
    async with websockets.connect(uri) as websocket:
        custom_print("[INFO] Connected to the server")
        ...
        custom_print(f"[INFO] Server response (DH key generation): {response}")
        ...
        if response.get("status") == "success" and response.get("key") == "server_public_key":
            ...
            response = json.loads(await websocket.recv())
            custom_print(f"[INFO] Server response (attestation): {response}")
            ...
            if response.get("status") == "success" and response.get("key") == "attest":
                ...
                custom_print(f"[INFO] Ciphertext in Base64: {ciphertext_b64}")
                ...
                response = json.loads(await websocket.recv())
                custom_print(f"[INFO] Server response (secret decryption): {response}")
                ...
                if response.get("status") == "success" and response.get("key") == "secret_decryption":
                    ...
                    if actual_secret == secret:
                        custom_print(f"[INFO] Server successfully decrypted the secret")
                    else:
                        custom_print(f"[ERROR] Server is not running the expected code")
                        sys.exit(0)
                ...
                response = json.loads(await websocket.recv())
                custom_print(f"[INFO] Server response (receive data): {response}")
                ...
                if response.get("status") == "success" and response.get("key") == "receive_data":
                    ...
                    encrypted_credentials = encrypt_data(cred, shared_key)
                    await websocket.send(json.dumps(["credentials", encrypted_credentials.hex()]))
                    response = json.loads(await websocket.recv())
                    custom_print(f"[INFO] Server response (credentials): {response}")
                    ...
                    if response.get("status") == "success" and response.get("key") == "credentials":
                        ...
                        response = json.loads(await websocket.recv())
                        custom_print(f"[INFO] Server response (client hello): {response}")
                        ...
                        if response.get("status") == "success" and response.get("key") == "email_response":
                            ...
                            custom_print("[INFO] Email response received")
                            ...
                            custom_print(f"[INFO] Elapsed time: {elapsed_time} seconds")
                            return elapsed_time
